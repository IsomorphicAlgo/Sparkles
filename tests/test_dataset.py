...
def test_build_feature_matrix_respects_feature_toggles() -> None:
    tz = "America/New_York"
    idx = pd.date_range("2024-06-03 09:30", periods=4, freq="1min", tz=tz)
    ohlcv = pd.DataFrame(
        {
...
def test_build_feature_matrix_ohlcv_close_only_when_intraday_off() -> None:
    tz = "America/New_York"
    idx = pd.date_range("2024-06-03 09:30", periods=2, freq="1min", tz=tz)
    ohlcv = pd.DataFrame({"close": [10.0, 10.0]}, index=idx)
    labels = pd.DataFrame(
        {
            "entry_close": [10.0, 10.0],
            "barrier_outcome": ["vertical", "take_profit"],
