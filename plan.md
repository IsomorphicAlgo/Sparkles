...
| 2026-06-21 | **Champion uniqueness preset + grid search:** **`rklb_daytrade_champion_uniqueness_v1.yaml`** (champion + I4); **`scripts/run_grid_search.py`** + **`configs/experiments/grids/`** for cartesian YAML knob sweeps; notebook **`RUN_MODE`** single/grid; tests **`test_grid_search.py`**. | `configs/experiments/presets/`, `configs/experiments/grids/`, `sparkles/config/grid.py`, `scripts/run_grid_search.py`, `notebooks/sparkles_train_console.ipynb`, `DEVELOPER.md`, `scripts/README.md`, `plan.md` | **Research / trials** |
