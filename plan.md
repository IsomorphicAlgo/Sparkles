...
| 2026-06-20 | **Incremental ingest cache:** one Parquet per symbol/interval (`{SYMBOL}_1min.parquet`); append missing days on `data_end` extend; legacy dated files still read; slice at load. | `sparkles/data/ingest.py`, `sparkles/labels/triple_barrier.py`, `sparkles/models/train.py`, `sparkles/features/market_data.py`, `tests/test_ingest_*.py`, `DEVELOPER.md`, `METHODOLOGY.md`, `plan.md` | **Ingest UX** (credit preservation) |
| 2026-06-20 | **Ingest cache lookup fix:** glob legacy dated Parquet when YAML `data_end` changes; labeled-cache fallback + tz align on load (fixes “no data” / index mismatch). | `sparkles/data/ingest.py`, `sparkles/labels/triple_barrier.py`, `tests/`, `plan.md` | **Ingest UX bugfix** |
| 2026-06-20 | **Ingest weekend/holiday skip:** TwelveData “No data is available” on empty calendar chunks (e.g. Sat after Fri close) → empty chunk, continue; restore `ingest_chunk_calendar_days: 10` in v3. | `sparkles/data/twelvedata_client.py`, `sparkles/data/retry.py`, `configs/experiments/rklb_daytrade_v3.yaml`, `tests/test_retry_credits.py`, `plan.md` | **Ingest UX bugfix** |
